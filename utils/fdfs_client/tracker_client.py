...
class Storage_info(object):
    ...
    def set_info(self, bytes_stream):
        (self.status, self.id, ip_addr, domain_name, self.src_id, version, join_time, up_time, totalMB, freeMB,
         ...
         self.if_trunk_server,) = bytesListToStrList(struct.unpack(self.fmt, bytes_stream))
        try:
...
class Group_info(object):
    ...
    def set_info(self, bytes_stream):
        (group_name, totalMB, freeMB, trunk_freeMB, self.count, self.storage_port, self.store_http_port,
         ...
         self.curr_trunk_file_id) = bytesListToStrList(struct.unpack(self.fmt, bytes_stream))
        try:
...
class Tracker_client(object):
    ...
    def tracker_list_servers(self, group_name, storage_ip=None):
        ...
        try:
            th.send_header(conn)
            send_buffer = struct.pack(group_fmt, group_name.encode()) + struct.pack(storage_ip_fmt, store_ip_addr.encode())
    ...
    def tracker_query_storage_stor_without_group(self):
        ...
        (group_name, ip_addr, store_serv.port, store_serv.store_path_index) = bytesListToStrList(struct.unpack(recv_fmt, recv_buffer))
        store_serv.group_name = group_name
        store_serv.ip_addr = ip_addr
        return store_serv
    ...
    def tracker_query_storage_stor_with_group(self, group_name):
        ...
        recv_fmt = '!%ds %ds Q B' % (FDFS_GROUP_NAME_MAX_LEN, IP_ADDRESS_SIZE - 1)
        store_serv = Storage_server()
        (group, ip_addr, store_serv.port, store_serv.store_path_index) = bytesListToStrList(struct.unpack(recv_fmt, recv_buffer))
        store_serv.group_name = group
        store_serv.ip_addr = ip_addr
        return store_serv
    ...
    def _tracker_do_query_storage(self, group_name, filename, cmd):
        ...
        store_serv = Storage_server()
        (group_name, ipaddr, store_serv.port) = bytesListToStrList(struct.unpack(recv_fmt, recv_buffer))
        store_serv.group_name = group_name
        store_serv.ip_addr = ipaddr
        return store_serv
